        ...
        if interaction.guild.fetch_member(member.id) is not None:
            try:
                success = discord.Embed(
                    title='[유저 관리] - 밴 ({})'.format(member.name),
                    description='해당 유저가 밴 되었 습니다.',
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(ban(bot), guilds=[discord.Object(id=947022293664673842)])
